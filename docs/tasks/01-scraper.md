...
- **2025-12-19:**
    - Playwright를 사용해 `#fundNm` 입력 및 `#btnSear` 클릭 후 동적 그리드 데이터 추출 성공.
    - WebSquare 그리드의 복잡성(다중 테이블 구조)을 해결하기 위해, 모든 행의 텍스트를 파이프(`|`)로 결합한 뒤 인덱싱하여 매핑하는 방식으로 데이터 수집 구현.
    - 프레임 중복 탐색으로 인한 데이터 중복 문제를 해결하기 위해 펀드명(공백제거) 기반의 엄격한 중복 제거(`seen_funds`) 로직 추가.
    - 총 7개 펀드의 보수 및 수수료 정보 CSV 저장 완료.
